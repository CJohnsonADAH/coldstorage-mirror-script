...
Process {
    ...
    Write-Progress -Activity "Considering for Validation" -Status $PackageFullName
    ...
    If ( $Pending ) {
        If ( Test-Path -LiteralPath $Logs -PathType Container ) {
            Get-ChildItem -LiteralPath $Logs -Recurse -File |? {
                ...
                Write-Progress -Activity "Considering for Validation" -Status ( "{0}: {1}" -f $PackageFullName,$_.FullName )
